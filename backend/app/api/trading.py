...
def account_overview():
    ...
    return AccountOverviewResponse(**kis_client.get_account_overview())


@router.get("/balance", response_model=BalanceResponse)
def balance():
    return BalanceResponse(**kis_client.get_account_balance())


@router.get("/holdings", response_model=list[HoldingItem])
def holdings():
    return [HoldingItem(**h) for h in kis_client.get_account_holdings()]


@router.get("/orders/pending", response_model=list[PendingOrder])
def pending_orders():
    return [PendingOrder(**o) for o in kis_client.get_pending_orders()]
...
def place_order(body: OrderRequest):
    ...
    if body.side.lower() == "buy":
        res = kis_client.place_buy_order(body.symbol, qty, price=price)  # type: ignore[arg-type]
    else:
        res = kis_client.place_sell_order(body.symbol, qty, price=price)  # type: ignore[arg-type]
    return OrderResponse(**res)


@router.post("/orders/cancel", response_model=OrderResponse)
def cancel(body: CancelOrderRequest):
    return OrderResponse(**kis_client.cancel_order(body.symbol, body.order_id))
